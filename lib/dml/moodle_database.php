...
abstract class moodle_database {
    ...
    public function synchLog() {
        ...
        if (isset($type[$this->last_type])) {
            $sql = explode(' ', strtolower($this->last_sql));
            $table = '';
            if ($sql[0] == 'update') {
                $table = trim($sql[1]);
            } else if ($sql[0] == 'insert' || $sql[0] == 'delete') {
                $table = trim($sql[2]);
            }
    ...
    protected function normalise_limit_from_num($limitfrom, $limitnum) {
        ...
        if ($CFG->debugdeveloper) {
            if (!is_numeric($limitfrom)) {
                ...
                debugging("Non-numeric limitfrom parameter detected: $strvalue, did you pass the correct arguments?", DEBUG_DEVELOPER);
            } else if ($limitfrom < 0) {
                debugging("Negative limitfrom parameter detected: $limitfrom, did you pass the correct arguments?", DEBUG_DEVELOPER);
            }

            if (!is_numeric($limitnum)) {
                $strvalue = var_export($limitnum, true);
                debugging("Non-numeric limitnum parameter detected: $strvalue, did you pass the correct arguments?", DEBUG_DEVELOPER);
            } else if ($limitnum < 0) {
                debugging("Negative limitnum parameter detected: $limitnum, did you pass the correct arguments?", DEBUG_DEVELOPER);
            }
        }
